...
- Phase 15 revision — floating "voice sidebar" root-caused (2026-07-17, same branch). What was wrong: a vertical floating pill over the tutor input. Root cause: **Gboard's hardware-keyboard companion toolbar** — the emulator advertises the host's physical keyboard, so Gboard hides the on-screen QWERTY and floats its toolbar (proof: the pill's own ☰ menu lists Gboard items like "Show on-screen keyboard"). It is NOT app UI — grep shows zero floating/overlay code in `lib/presentation/`; the tutor input is exactly [Mic][TextField][Send]. What changed: nothing in the app (nothing existed to delete); device fixed via `settings put secure show_ime_with_hard_keyboard 1` + Gboard restart. How verified: screenshots — docked QWERTY opens on focus, typing enters the field, pill gone; setting persists (=1). All other acceptance items re-confirmed: Piper speaking (logcat, Phase 15 final), barge-in (gen-token, logcat 14b), clean spoken text (unit-tested), auto chapter flow + optional quiz + in-reader Companion (device-verified Phase 15). 204 tests; analyze clean.

- Phase 15 FINAL — real Piper voice + flagship novel (2026-07-17, branch `feature/phase15-premium-offline-voice-final`, includes all prior UX phases):
  - **Piper TTS actually speaking** (not a scaffold): sherpa_onnx (ONNX runtime) runs the Piper VITS es-ES voice on-device. First-use download (63 MB model + espeak-ng data, progress bar in Voice Settings), pure-Dart tar.bz2 extract, cached in app documents (not in git). Sentence-chunked synthesis → WAV → audioplayers, generation-token cancellation (barge-in). Piper = default engine; device TTS = user-selectable fallback only.
  - **Verified on device (logcat + screenshots):** model files on device (`es_ES-davefx-medium.onnx` = 63,149,192 bytes), synthesis lines (`[PIPER] gen#4 chunk0 samples=39680 sr=22050 <<Hola, soy tu profesor…>>`), chunk timing == audio duration (playback ran), "Piper voice ready" card, reader narration in Playing state with audio-focus events.
  - **Flagship novel "La casa del faro"**: original graded A2 novella — 7 chapters, 42 continuous pages, ~2,700 Spanish words + full English, recurring characters, dialogue, cliffhangers, optional quiz. Story model gains chapters; reader shows "Capítulo N · title"; reading flows chapter-to-chapter with the completion card only at the end. Verified on device (library card "17 min · 7 chapters"; reader 1/42 with chapter header).
  - 204 tests green; analyze clean; core zero-diff.
  - **Honest limitations:** (1) Piper synthesis runs on the main isolate — a long paragraph can jank the UI for a moment on the emulator while generating (move to a background isolate later); (2) English Piper voice defined but not downloaded/tested (es first); (3) the novel is ~17–20 min of pure reading (learner pace with translations/audio ≈ 30 min) — below the 30–45 min pure-reading target; growing it further is straight content authoring; (4) remaining-time items (text size / line spacing / sepia, tutor personality) not reached — stopped after the two priority deliverables per instructions; (5) emulator audio can't be recorded, so "how it sounds" is proven by synthesis+playback telemetry, not a recording.
